...
namespace HwaUtil {
    double Timer::program_start_time=0.0;
    ...
    std::ostream &Timer::print_time_usage(std::ostream &os) {
        ...
        auto prog_total_time = program_total_time;
        os //<< "\033[32m"
        ...
        for (auto &func_time: func_time_info) {
            auto func_total_time = double(func_time.second.total_time());
        ...
        os << std::setfill('-') << std::setw(w_calls + w_avg + w_time + w_func_name + w_class_name + w_per) << "-"
           << std::endl;
        return os;
    }
    ...
    double Timer::func_time(const std::string &class_name, const std::string &function_name) {
        std::string func_name = class_name + "|||" + function_name;
    ...
    double Timer::FuncTimeInfo::total_time() {
